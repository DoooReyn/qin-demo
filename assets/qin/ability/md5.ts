...
class Md5 {
  ...
  /**
   * Initialize buffer to be hashed
   */
  public start() {
    this._dataLength = 0;
 ...
 * MD5 计算能力接口
 */
export interface IMD5 extends IAbility {
  /**
   * 计算 md5 值
   * @param data UTF-8 字符串
   */
  utf8(data: string): string
  /**
   * 计算 md5 值
   * @param data ASCII 字符串
   */;
  ascii(data: string): string;
}
...
export const md5: IMD5 = {
  name: "MD5",
  description: "MD5 计算能力",
  utf8(data: string) {
    return Md5.hashStr(data);
  },
  ascii(data: string) {
    return Md5.hashAsciiStr(data);
  },
};

/**
 * The Grad class represents a gradient vector for Perlin noise and Simplex noise.
 * It has three components: x, y, and z.
 *
 * The Grad class has the following properties:
 *
 * - x: The x component of the gradient vector.
 * - y: The y component of the gradient vector.
 * - z: The z component of the gradient vector.
 *
 * The Grad class has the following methods:
 *
 * - Dot2(x: number, y: number): number - Calculates the dot product of the gradient vector with another vector (x, y).
 * - Dot3(x: number, y: number, z: number): number - Calculates the dot product of the gradient vector with another vector (x, y, z).
 */
class Grad {
    public x: number;
    public y: number;
...
/**
 * 噪声模块
 * @description 提供多维噪声实现的核心类，用于生成 Perlin/Simplex 噪声。
 * - 包含 2D/3D Simplex 和 Perlin 噪声实现
 * - 提供 seed、simplex2、simplex3、perlin2、perlin3 等方法
 */
export class Noise {
    private grad3: Grad[] = [
        new Grad(1, 1, 0),
